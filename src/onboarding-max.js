...
async function generateReportFeedback(report, userPlan, openai) {
  ...
  const sysPrompt = `
Ты — опытный тренер, который анализирует отчёты клиентов и даёт развёрнутый фидбэк.

ЗАДАЧА:
- Проанализируй отчёт клиента в контексте его плана
- Дай развёрнутый, мотивирующий фидбэк
- Отметь прогресс и достижения
- Дай конкретные советы по улучшению
- Будь человечным и поддерживающим

СТРУКТУРА ФИДБЭКА:
1. Позитивная оценка (что сделано хорошо)
2. Анализ прогресса относительно плана
3. Конкретные рекомендации
4. Мотивация на продолжение

ВЫВОД: Только текст фидбэка, без дополнительных комментариев.
`;
  ...
  bot.on('message', async (msg) => {
    ...
    if (u?.plan && (msg.photo || msg.document)) {
      ...
      try {
        ...
        if (feedback) {
          await sendMsg(bot, chatId, `📊 <b>Анализ твоей активности:</b>\n\n${feedback}`, { parse_mode: 'HTML' });
        } else {
    ...
    if (u?.plan && u?.awaitingReport) {
      ...
      try {
        ...
        if (feedback) {
          ...
          await sendMsg(bot, chatId, `Текстовый отчёт сохранён. Продолжай тренировки по плану! 💪`);
        }
      } catch (e) {
        console.error('Feedback generation error', e);
        await sendMsg(bot, chatId, `Текстовый отчёт сохранён. Продолжай тренировки по плану! 💪`);
